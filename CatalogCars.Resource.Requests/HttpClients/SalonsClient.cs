...
namespace CatalogCars.Resource.Requests.HttpClients
{
    public class SalonsClient : BaseHttpClient
    {
        ...
        {
            return await _client.PostAsync("add", new StringContent(JsonConvert.SerializeObject(salon),
                Encoding.UTF8, "application/json"));
        }

        public async Task<HttpResponseMessage> UpdateSalonAsync(Salon salon)
        {
            return await _client.PutAsync("update", new StringContent(JsonConvert.SerializeObject(salon),
                Encoding.UTF8, "application/json"));
        }
