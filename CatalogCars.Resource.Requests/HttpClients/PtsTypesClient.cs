...
namespace CatalogCars.Resource.Requests.HttpClients
{
    public class PtsTypesClient : BaseHttpClient
    {
        ...
        {
            return await _client.PostAsync("save", new StringContent(JsonConvert.SerializeObject(ptsType),
                Encoding.UTF8, "application/json"));
        }

        public async Task<HttpResponseMessage> UpdatePtsTypeAsync(PtsType ptsType)
        {
            return await _client.PutAsync("save", new StringContent(JsonConvert.SerializeObject(ptsType),
                Encoding.UTF8, "application/json"));
        }
