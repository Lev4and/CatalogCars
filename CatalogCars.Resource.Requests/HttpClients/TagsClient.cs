...
namespace CatalogCars.Resource.Requests.HttpClients
{
    public class TagsClient : BaseHttpClient
    {
        ...
        {
            return await _client.PostAsync("save", new StringContent(JsonConvert.SerializeObject(tag),
                Encoding.UTF8, "application/json"));
        }

        public async Task<HttpResponseMessage> UpdateTagAsync(Tag tag)
        {
            return await _client.PutAsync("save", new StringContent(JsonConvert.SerializeObject(tag),
                Encoding.UTF8, "application/json"));
        }
