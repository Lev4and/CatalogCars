...
namespace CatalogCars.Resource.Requests.HttpClients
{
    public class PhotoClassesClient : BaseHttpClient
    {
        ...
        {
            return await _client.PostAsync("add", new StringContent(JsonConvert.SerializeObject(photoClass),
                Encoding.UTF8, "application/json"));
        }

        public async Task<HttpResponseMessage> UpdatePhotoClassAsync(PhotoClass photoClass)
        {
            return await _client.PutAsync("update", new StringContent(JsonConvert.SerializeObject(photoClass),
                Encoding.UTF8, "application/json"));
        }
