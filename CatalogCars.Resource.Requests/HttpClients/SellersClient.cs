...
namespace CatalogCars.Resource.Requests.HttpClients
{
    public class SellersClient : BaseHttpClient
    {
        ...
        {
            return await _client.PostAsync("add", new StringContent(JsonConvert.SerializeObject(seller),
                Encoding.UTF8, "application/json"));
        }

        public async Task<HttpResponseMessage> UpdateSellerAsync(Seller seller)
        {
            return await _client.PutAsync("update", new StringContent(JsonConvert.SerializeObject(seller),
                Encoding.UTF8, "application/json"));
        }
