...
namespace CatalogCars.WebApplication.Areas.Admin.Controllers
{
    ...
    public class SectionsController : Controller
    {
        ...
        {
            return View("Edit", await _sectionsRequester.GetSectionAsync(id));
        }

        [HttpPost]
        [Route("~/[area]/[controller]/[action]")]
        public async Task<IActionResult> Save([FromForm] Section viewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    saveResult = await _sectionsRequester.AddSectionAsync(viewModel);
                }
                else
                {
                    saveResult = await _sectionsRequester.UpdateSectionAsync(viewModel);
                }
        ...
        {
            await _sectionsRequester.DeleteSectionAsync(id);
