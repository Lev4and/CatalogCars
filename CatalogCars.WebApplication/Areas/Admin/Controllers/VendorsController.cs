...
namespace CatalogCars.WebApplication.Areas.Admin.Controllers
{
    ...
    public class VendorsController : Controller
    {
        ...
        {
            return View("Edit", await _vendorsRequester.GetVendorAsync(id));
        }

        [HttpPost]
        [Route("~/[area]/[controller]/[action]")]
        public async Task<IActionResult> Save([FromForm] Vendor viewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    saveResult = await _vendorsRequester.AddVendorAsync(viewModel);
                }
                else
                {
                    saveResult = await _vendorsRequester.UpdateVendorAsync(viewModel);
                }
        ...
        {
            await _vendorsRequester.DeleteVendorAsync(id);
